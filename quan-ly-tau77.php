...
if ($is_admin && $_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'add') {
        ...
        $status = $_POST['status'];
        $is_locked = isset($_POST['is_locked']) ? (int)$POST['is_locked'] : 0;
        ...
        if ($check->fetchColumn() > 0) {
            ...
            $sql = "INSERT INTO ships (project_name, fac, ship_type, pic, group_name, man_hours, status, delivery_date, is_locked) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $conn->prepare($sql)->execute([$project, $fac, $type, $pic, $group, $hours, $status, $date, $is_locked]);
    ...
    elseif ($_POST['action'] == 'update_full') {
        ...
        if ($check->fetchColumn() > 0) {
            ...
            $sql = "UPDATE ships SET project_name = ?, fac = ?, ship_type = ?, pic = ?, group_name = ?, man_hours = ?, status = ?, delivery_date = ?, is_locked = ? WHERE id = ?";
            $conn->prepare($sql)->execute([$project, $fac, $type, $pic, $group, $hours, $status, $date, $is_locked, $id]);
            header("Location: " . $current_page);
            exit();
        }
    }
...
// Tại trang quản lý tàu này, không lọc `is_locked = 0` để Admin có thể nhìn thấy cấu hình mở khóa lại tàu.
...
/* Giao diện Nhập liệu: Viền đậm xanh lá, nền xanh lá nhạt */
.form-add { background:#f3faf4; border:2px solid #28a745; }

/* Giao diện Tìm kiếm: Viền đậm xanh dương, nền xanh dương nhạt */
.form-search { background:#f0f7ff; border:2px solid #007bff; }

.form-row{ display:flex; flex-wrap:wrap; gap:15px; align-items:flex-end; }
.form-group{ display:flex; flex-direction:column; flex:1 1 140px; }
.form-group label{ font-size:11px; font-weight:bold; color:#444; margin-bottom:3px; text-transform:uppercase; }
input, select{ padding:8px; border:1px solid #ccc; border-radius:4px; font-size:13px; width:100%; box-sizing:border-box; background-color: #fff; }
...
th{ background-color:#28a745; color:white; padding:12px; text-align:center; font-size:13px; font-weight:bold; }
td{ padding:10px; font-size:14px; text-align:center; background-color:#fff; }

/* Định nghĩa chiều rộng cụ thể cho từng cột */
.col-stt { width: 35px; text-align: right;}
.col-name { width: 65px; text-align: center; }
.col-fac { width: 45px; text-align: center; }
.col-type { width: 130px; text-align: left; }
.col-pic { width: 100px; text-align: left; }
.col-group { width: 45px; text-align: center;}
.col-hours-pp { width: 55px; text-align: right; }
.col-hours-act { width: 55px; text-align: right; }
.col-pre { width: 55px; text-align: right; }
.col-eff { width: 45px; text-align: right; }
.col-prog { width: 45px; text-align: right; }
.col-insp { width: 45px; text-align: center; }
.col-status { width: 75px; text-align: center; }
.col-start-date { width: 80px; text-align: center; } 
.col-date { width: 80px; text-align: center; }
.col-lock { width: 65px; text-align: center; }
.col-action { width: 100px; text-align: center; }

.row-chua-thi-cong td{ background-color:#ffc1cc !important; }
.row-dang-thi-cong td{ background-color:#fff3cd !important; }
.row-da-ban-giao td{ background-color:#d4edda !important; }
...
.pagination a{ padding:8px 12px; border:1px solid #ddd; text-decoration:none; color:#28a745; border-radius:4px; background:#fff; }
...
.badge-lock { background: #fff0f0; color: #e53935; padding: 4px 8px; border-radius: 5px; font-weight: bold; font-size: 12px; border: 1px solid #ffcdd2; }
.badge-active { background: #e3f2fd; color: #1e88e5; padding: 4px 8px; border-radius: 5px; font-weight: bold; font-size: 12px; border: 1px solid #bbdefb; }
...
<div class="container">
    ...
    <div class="form-box form-add">
        <form method="POST">
            ...
            <div class="form-row">
                ...
                <div class="form-group"><label>Khóa tàu</label>
    ...
    <div class="form-box form-search">
    ...
    <div class="table-wrapper">
        <table>
            ...
            <?php if (count($ships) > 0): $stt = $offset + 1; ?>
                    ...
                    <?php endif; ?>
                        ...
                        <td class="col-lock">
                            <span class="view-mode">
                                <?php echo ($ship['is_locked'] == 1) ? '<span class="badge-lock">Khóa</span>' : '<span class="badge-active">Hoạt động</span>'; ?>
                            </span>
                            <?php if($is_admin): ?>
                            <select name="is_locked" class="edit-mode">
                                <option value="0" <?php echo ($ship['is_locked'] == 0) ? 'selected' : ''; ?>>Hoạt động</option>
                                <option value="1" <?php echo ($ship['is_locked'] == 1) ? 'selected' : ''; ?>>Khóa</option>
                            </select>
                            <?php endif; ?>
                        </td>
                        <?php if($is_admin): ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="<?php echo $is_admin ? '17' : '16'; ?>" style="text-align:center;">Dữ liệu trống hoặc không tìm thấy kết quả phù hợp.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
