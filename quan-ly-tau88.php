...
// TRUY VẤN DỮ LIỆU TỔNG HỢP (ĐÃ CẬP NHẬT LOGIC ĐƯA CHỜ DUYỆT LÊN TRÊN CÙNG TRANG MỚI NHẤT)
$query = "
SELECT s.*, 
...
ORDER BY 
    status_order ASC, 
    CASE WHEN s.status = 'Chờ duyệt' THEN s.id END DESC,  -- Tàu chờ duyệt mới nhất lên trên cùng đầu tiên
    s.project_name ASC                                     -- Các trạng thái khác sắp xếp theo Alphabet tên tàu
            ...
            <?php if (count($ships) > 0): $stt = $offset + 1; ?>
                ...
                <tr id="row-<?php echo $ship['id']; ?>" class="<?php echo $status_class; ?>" onclick="selectRow(this)">
                        ...
                        <td class="col-date">
                            ...
                            <?php if($is_admin): ?>
                            <div id="edit-date-container-<?php echo $ship['id']; ?>" class="edit-mode" style="display: <?php echo ($ship['status'] == 'Đã bàn giao') ? 'block' : 'none'; ?>;">
...
function toggleDateAdd(selectId, containerId){
    ...
    document.getElementById(containerId).style.display = (status === "Đã bàn giao") ? "block" : "none";
}
