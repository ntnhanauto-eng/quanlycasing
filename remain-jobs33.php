...
<style>
    ...
    .header-flex { border-bottom: 2px solid var(--primary); padding-bottom: 10px; margin-bottom: 20px; }
    .header-flex h2 { margin: 0; color: var(--primary); font-size: 18px; text-transform: uppercase; text-align: center; }
    ...
    /* Thiết lập cho các ô textarea nhập liệu có kích thước ổn định, cho phép kéo giãn dọc */
    textarea { resize: vertical; box-sizing: border-box; width: 100%; min-height: 35px; }
    
    .table-responsive { overflow-x: auto; margin-top: 10px; border: 1px solid var(--border); border-radius: 4px; }
    table { width: 100%; border-collapse: collapse; min-width: 1300px; }
    th { background: var(--primary); color: #fff; padding: 12px 8px; border: 1px solid #388e3c; font-size: 11px; }
    ...
    .status-da-xong { background-color: #d4edda !important; }
    tr:hover { background-color: #f5f5f5; cursor: pointer; }
    tr.selected { background-color: #eceff1 !important; border-left: 4px solid var(--primary); }
    .edit-mode { display: none; }
    tr.editing .view-mode { display: none; }
    tr.editing .edit-mode { display: block; width: 100%; box-sizing: border-box; }
    tr.editing textarea.edit-mode { display: block; width: 100%; box-sizing: border-box; resize: vertical; min-height: 60px; }
    .btn { padding: 8px 15px; border: none; border-radius: 4px; cursor: pointer; font-size: 11px; font-weight: bold; color: #fff; text-decoration: none; text-align: center; }
    ...
    /* Cấu hình CSS phục vụ cho tính năng IN (Khổ ngang A4) */
    @media print {
        @page { size: A4 landscape; margin: 10mm; }
        body { background: #fff; color: #000; padding: 0; margin: 0; font-size: 11px; }
        .container { box-shadow: none; padding: 0; }
        header, footer, .sidebar, .admin-box, .search-box, .pagination, .btn, th:last-child, td:last-child { display: none !important; }
        .table-responsive { overflow: visible; border: none; margin: 0; }
        table { width: 100%; min-width: 100% !important; table-layout: fixed; word-wrap: break-word; }
        th, td { border: 1px solid #000 !important; padding: 6px 4px !important; font-size: 10px !important; background: transparent !important; color: #000 !important; }
    ...
    <div class="admin-box">
        <form method="POST" action="remain-jobs.php?<?=$query_string?>">
            ...
            <div class="form-row">
                ...
                <div class="form-group" style="flex:2"><label>TÊN VẬT TƯ</label><textarea name="material_name" rows="3" required></textarea></div>
            ...
            <div class="form-row" style="margin-top:10px;">
                ...
                <div class="form-group" style="flex:2"><label>Ghi chú</label><textarea name="notes" rows="3"></textarea></div>
    ...
    <div class="search-box">
        <form method="GET" action="remain-jobs.php">
            <div class="form-row">
                ...
                <?php if ($is_logged_in): ?>
                    <button type="button" class="btn btn-print" style="height:35px;" onclick="window.print()">IN BÁO CÁO</button>
                    <button type="button" class="btn btn-excel" style="height:35px;" onclick="exportToExcel()">XUẤT EXCEL</button>
                <?php endif; ?>
            </div>
        </form>
    </div>
        ...
        <table id="dataTable">
            <thead>
                <tr>
                    ...
                    <th>POR</th>
                    <th>PART NO.</th>
                    <th class="col-material">TÊN VẬT TƯ</th>
                    <th width="40">S.L</th>
                    <th>LỒI DO</th>
                    <th>VẬT TƯ VỀ</th>
                    <th>NGÀY XONG</th>
